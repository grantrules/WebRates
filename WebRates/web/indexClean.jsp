...
var Rate = {
	...
	updatePrices : function(prices) {
		setOldPrices(getPrices());
		setPrices(prices);
		redraw();
	},

	setPrices : function(p) { prices = p; },
	getPrices : function() { return prices; },

	setOldPrices : function(p) { oldPrices = p; },
	getOldPrices : function() { return oldPrices; },

	redraw : function() {
		htmlObj.innerHTML = generateHTML();
	},
	...
	comparePrices : function(old, notold, values) {
	...
	/* constructor, needs to realize it's undrawn & load props */
}

var PrettyRate = {
            
        create : function() {
            this.htmlObj = RateContainer.newRate();
            redraw();
        },

        generateHTML : function() {
            var tb = Rate.fixp(prices.bid, prices.precision);
            var ta = Rate.fixp(prices.ask, prices.precision);
            return '<div class="pairContainer"><span class="pairName"><h1>'+prices.symbol+'</h1></span><span class="pairTime"><h2>10:53:18</h2></span><div class="'+compareprices(oldPrices.bid, prices.bid, [ 'green', '', 'red' ])+'"><h3>Low: '+Rate.fixp(prices.low,prices.precision)+'</h3><table align="center" border="0" cellpadding="2" cellspacing="0"><tr><td valign="baseline" class="numbers"><h1>'+getfromright(tb,3)+'</h1></td><td valign="baseline" class="numbers"><h2>'+getfromright(tb,2)+'</h2></td><td valign="top" class="numbers"><h3>'+getfromright(tb,1)+'</h3></td></tr></table></div><div class="'+compareprices(oldPrices.ask, prices.ask, [ 'green', '', 'red' ])+'"><h3>High: '+Rate.fixp(p.high,p.precision)+'</h3><table align="center" border="0" cellpadding="2" cellspacing="0"><tr><td valign="baseline" class="numbers"><h1>'+getfromright(ta,3)+'</h1></td><td valign="baseline" class="numbers"><h2>'+getfromright(ta,2)+'</h2></td><td valign="top" class="numbers"><h3>'+getfromright(ta,1)+'</h3></td></tr></table></div></div>';
        },
        
        select : function() {
            htmlObj.className = 'selected';
            this.selected = true;
        },
        ...
        isHidden : function() {
            return (htmlObj.style.display == 'none' ? true : false);
        }

}
Object.extend(Rate, PrettyRate);
...
var RateController = {
	...
	findBySymbol : function(symbol) {
		...
		return this.createRate(s);
	},

	createRate : function(symbol) {
		var r = new Rate();
		r.symbol = symbol;
		rates.push(r);
		return r;
...
var ProfileController = {
	...
	save : function() {
		var str = toJSON(profiles);
		// ajax req
	},

	load : function(str) {
		this.profiles = fromJSON(str);
	}
